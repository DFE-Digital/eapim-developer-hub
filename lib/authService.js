...
export const getOAuthToken = (res) => {
  if (res) {
    return res.req.cookies.access_token || res.locals.access_token
  }
...
export const signOut = async (redirectUri) => {
  ...
  const myMSALObj = new Msal.UserAgentApplication(msalConfig(authConfig))
  myMSALObj.logout()
}

export const signInRedeem = async (idTokenHint) => {
  const myMSALObj = new Msal.UserAgentApplication(config.verify)
  myMSALObj.loginRedirect({ ...loginRequest, extraQueryParameters: { id_token_hint: idTokenHint } })
}

export const register = async () => {
  const myMSALObj = new Msal.UserAgentApplication(config.register)
  myMSALObj.loginRedirect(loginRequest)
}
...
export const forgotPassword = async () => {
  const myMSALObj = new Msal.UserAgentApplication(config.forgotPassword)
  myMSALObj.loginRedirect({ authority: config.forgotPassword.auth.authority })
}

export const changePassword = async () => {
  const myMSALObj = new Msal.UserAgentApplication(config.changePassword)
